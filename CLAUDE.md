...
> **Note:** The `@kitnai/*` names above are internal workspace package names. User projects import from the published npm scope: `@kitn/core` (maps to `@kitnai/core`) and `@kitn/hono-routes` (maps to `@kitnai/hono`).
...
User projects import from the published npm packages: `@kitn/core` and `@kitn/hono-routes`.
